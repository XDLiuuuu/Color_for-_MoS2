...
def reflectance(lam,th):
    x = 1240/lam                                           ##Convert the wavelength to the energy.
    r = reflec_absorp_func([0.65],100, th, 'p', 330, 800)    ##The range of the wavelength must include the range of the visible light.
    re = r(x)
    return re
...
for i in range(46):
    ...
    ax.add_patch(circle)
    if th%((np.pi)/4)==0:
        ax.annotate('{:2f} '.format(th), xy=(x, y*1.2-2.5), va='center',
